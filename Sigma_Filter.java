...
//    private static int roiy = 0;
    ...
    public int showDialog(ImagePlus imp, String command, PlugInFilterRunner pfr) {
        ...
        gd.addNumericField("Noise level", noise_level, 3);
        gd.addNumericField("De-noise strength", denoise_strength, 1);
...
//        minPixFraction = gd.getNextNumber();
        ...
        if (gd.invalidNumber() || radius<0.5 || radius>15 || denoise_strength < 0 || denoise_strength > 1)
            return false;
...
//            minPixNumber = (int)(kNPoints * minPixFraction + 0.999999); //min pixels in sigma range
        ...
        pass++;
        doFiltering((FloatProcessor)ip, kRadius, lineRadius, sigma, noise_level, denoise_strength, outlierAware);
    }
    ...
    // shifted.
    //
    public void doFiltering(FloatProcessor ip, int kRadius, int[] lineRadius, double sigma, double noise_level, double denoise_strength, boolean outlierAware) {
        ...
        int xminInside = Math.max(xmin, 0);
        int xmaxInside = Math.min(xmax, width);
        ...
        // sigma*noise
        double[] thr1 = new double[height*width];
        for(int y = roi.y; y<roi.y+roi.height; y++){
            for (int x = roi.x, p=x+y*width; x<xEnd; x++,p++){
                double photo_noise = Math.sqrt(pixels[p]);
                thr1[p] = Math.max(photo_noise, noise_level) * sigma;
            }
        }
        for (int y=roi.y; y<roi.y+roi.height; y++) {
